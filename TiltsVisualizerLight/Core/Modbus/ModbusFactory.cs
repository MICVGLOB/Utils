...
namespace Modbus.Core {
    ...
    public class ModbusReadUnit : ModbusUnitBase {
        protected static byte readFunction = 3;
        protected UInt16 quantity;
        UInt16 startAddress;
        ...
        public override TransmitUnit CreateTransmitUnit() {
            return new TransmitUnit(CreateTransmitData(), 2*GetActualQuantity() + 5, (x) => IsRecievedDataValid(x), index, stagesCount,
                transmitTimeout, receiveTimeout, errorMode, progressType);
        }
        protected virtual int GetActualQuantity() {
            return quantity;
        }
        ...
        bool IsRecievedDataValid(List<byte> receivedData) {
            if(receivedData.Count < 7 || (receivedData.Count - 5) != 2 * GetActualQuantity())
                return false;
            var crc = ModbusHelper.CRC_Calc16(receivedData.Take(receivedData.Count - 2).ToList());
            return ((byte)(crc >> 8) == receivedData[receivedData.Count - 2])
                && ((byte)(crc) == receivedData[receivedData.Count - 1])
                ...
                && (receivedData[2] == (byte)(2 * GetActualQuantity()));
        }
    }
    public class ModbusReadReportUnit : ModbusReadUnit {
        public ModbusReadReportUnit(string id, byte deviceAddress, UInt16 startAddress, UInt16 quantity, int stagesCount = 1, int index = 1,
            int transmitTimeout = 30, int receiveTimeout = 1000, SerialPortErrorMode errorMode = SerialPortErrorMode.ErrorOnTimeout,
            SerialPortProgressType progressType = SerialPortProgressType.Normal) : base(id, deviceAddress, startAddress, quantity, stagesCount, index, transmitTimeout, receiveTimeout, errorMode, progressType) {
        }
        protected override int GetActualQuantity() {
            return 19;
        }
    }
}
