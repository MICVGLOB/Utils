...
namespace ARMHexCorrector {
    public static class CRC {
        public static UInt32 CalculateCRC32(List<byte> data) {
            UInt32 crc = 0xFFFFFFFF;
            foreach(var item in data)
                crc = crc32_tab[(crc ^ item) & 0xFF] ^ (crc >> 8);
            return crc ^ 0xFFFFFFFF;
        }
        public static byte[] CreatePatch(UInt32 current, UInt32 desired) {
            int[] crc_idx = new int[4];
            byte[] bytes = new byte[4];

            UInt32 iter_crc = ~desired;
            for(int i = 3; i >= 0; --i) {
                crc_idx[i] = crc32_rtab[iter_crc >> 24];
                iter_crc = (iter_crc ^ crc32_tab[crc_idx[i]]) << 8;
            }

            UInt32 crc = ~current;
            for(int i = 0; i < 4; ++i) {
                bytes[i] = (byte)((crc ^ crc_idx[i]) & 0xff);
                crc = (crc >> 8) ^ crc32_tab[bytes[i] ^ (crc & 0xff)];
            }
            return bytes;
        }
