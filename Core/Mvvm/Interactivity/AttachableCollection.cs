...
namespace Mvvm.Core {
    public class AttachableCollection<T> : FreezableCollection<T>, IAttachableObject where T : DependencyObject, IAttachableObject {
        ...
        internal virtual void ItemAdded(T item) {
            if(ShouldAttachItem(item) && AssociatedObject != null)
                item.Attach(AssociatedObject);
        }
